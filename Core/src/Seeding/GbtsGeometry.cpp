...
bool GbtsLayer::verifyBin(const GbtsLayer* pL, std::uint32_t b1,
  ...
  if (m_layer->m_type == 0 && pL->m_layer->m_type != 0) {  // barrel <- endcap
    ...
    if(z0_max < minZ0-tol || z0_min > maxZ0+tol) { return false;
}
    return true;
  }
  if(m_layer->m_type != 0 && pL->m_layer->m_type != 0) {//endcap <- endcap
    std::cout<<"Jasper: end cap to end cap check"<<std::endl;
    float z2 = pL->m_layer->m_refCoord;
    ...
    if (z2 > 0) { // positive endcap
      ...
      if(z0_max < minZ0-tol) { return false;
}
      ...
      if (r2min > r1max) {
        ...
        if (z0_min > maxZ0+tol) { return false;
}
      }
    }
  ...
  std::cout<<"Jasper: endcap to barrel check"<<std::endl;
    float z1 = m_layer->m_refCoord;
    float r1max = m_maxBinCoord.at(b1);
    float r1min = m_minBinCoord.at(b1);
    ...
    bool beyond_range = (z0_max < minZ0-tol || z0_min > maxZ0+tol);
    ...
    beyond_range = (z0_max < minZ0-tol || z0_min > maxZ0+tol);
