...
SeedContainer2 GraphBasedTrackSeeder::createSeeds(
  ...
  std::vector<OutputSeedProperties> vOutputSeeds; //CHANGE THIS TO STRUCT I THINK
    ...
    for (std::uint32_t n1Idx = 0; n1Idx < B1.vn.size(); ++n1Idx) {
      ...
      for (auto& slw : phiSlidingWindow) {
		...
		if (isBarrel1 && isBarrel2 && isBarrel3) {
		  ...
		  if(!no_gap) {
		    addTauRatioCorr = m_cfg.tauRatioCorr;//assume more scattering due to the layer in between
		  }
		}
		else {
	      ...
	      if (isBarrel1 && isBarrel2 && isBarrel3) {//Pixel barrel
                ...
                if (!validateTriplet(sps, tripletPtMin, absTauRatio, cutTauRatioMax) ) { continue;
}
...
void GraphBasedTrackSeeder::extractSeedsFromTheGraph(
  ...
  //PULL TO CONGIF
  const float maxSeedSplitEta = 0.6;
  const float maxInvRadDiff       = 0.7e-2;//in inverse meters
  ...
  for (GbtsEdge* pS : vSeeds) {
    ...
    std::uint32_t seedSplitFlag = (seedEta < maxSeedSplitEta) && (origSeedSize > 3) && (origSeedSize <= 5) ? 1 : 0;

    if (seedSplitFlag != 0) {//split the seed by dropping spacepoints
    ...
    bool confirmed = diffs[0] < maxInvRadDiff && diffs[1] < maxInvRadDiff && diffs[2] < maxInvRadDiff;
...
bool GraphBasedTrackSeeder::validateTriplet(std::array<const GbtsNode*, 3>& sps, const float tripletMinPt, const float tauRatio, const float tauRatioCut) const {
  ...
  for(std::uint32_t k=0;k<2;k++) {
    ...
    const float dx = sps[spIdx]->x - x0;

    const float dy = sps[spIdx]->y - y0;
